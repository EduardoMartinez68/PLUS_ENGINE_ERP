...
def user_storage_used_gb(user) -> float:
    #get the total in bytes
    bytes_used = File.objects.filter(upload_user=user).aggregate(total=Sum('size'))['total'] or 0

    # convert to GB
    gb_used = bytes_used / (1024 ** 3)
    return round(gb_used, 2)

def user_storage_used(user)->int:
...
def can_upload(user, filesize)->bool:
    ...
    limit = get_limit_of_the_user(user) * 1024 * 1024 * 1024  # 1 GiB
    return user_storage_used(user) + filesize <= limit

def get_limit_of_the_user(user)->int:
    #first we will see if the user is in pc or in the web 
    if TYPE_VERSION=='DESKTOP':
        import shutil
    ...
    if sub:
        return sub.get_plan_storage() #this return 1 for 1 GB, 5 for 5 GB, etc.
...
def generate_unique_name(folder, filename):
...
def build_upload_path(user, folder, filename):

    return (
        f"uploads/"
        f"{user.company.id if user.company else 0}/"
        f"{user.branch.id if user.branch else 0}/"
        f"{folder.id}/"
        f"{filename}"
    )

def build_thumbnail_path(user, folder, filename):
...
def upload_multiple_files(
    ...
    folder = get_folder(user, folder_id)
    ...
    for uploaded_file in uploaded_files:
        ...
        relative_upload_path = build_upload_path(
            user,
            folder,
            file_name
        )

        absolute_upload_path = os.path.join(
            settings.MEDIA_ROOT,
            relative_upload_path
        )

        os.makedirs(
            os.path.dirname(absolute_upload_path),
            exist_ok=True
        )
        ...
        with open(absolute_upload_path, "wb+") as destination:
        ...
        file_instance.file.name = relative_upload_path
        ...
        try:
            ...
            thumb_url = generate_thumbnail_for_file(
                file_instance
            )

            if thumb_url:

                rel_thumb_path = thumb_url.replace(
                    settings.MEDIA_URL,
                    ""
                )

                file_instance.thumbnail.name = rel_thumb_path
        ...
        temp_encrypted_path = absolute_upload_path + ".encrypted"

        with open(absolute_upload_path, "rb") as source:

            with open(temp_encrypted_path, "wb") as encrypted_dest:
                ...
                while True:
                    ...
                    encrypted_dest.write(encrypted_chunk)

        # =====================================================
        # REPLACE ORIGINAL FILE
        # =====================================================

        os.remove(absolute_upload_path)

...
def get_information_of_the_medical_history_for_customer_id(user, customer_id:int)->Dict[str, Any]:
    ...
    data = {
        ...
        # Información médica
        "medical_info": {
            "skull": medical_info.skull if medical_info else '',
            "blood_type": medical_info.blood_type if medical_info else '',
            "height_cm": str(medical_info.height_cm) if medical_info and medical_info.height_cm else '',
            "weight_kg": str(medical_info.weight_kg) if medical_info and medical_info.weight_kg else '',
            "bmi": str(medical_info.bmi) if medical_info and medical_info.bmi else '',
            "chronic_conditions": medical_info.chronic_conditions if medical_info else '',
            "allergies": medical_info.allergies if medical_info else '',
            "medications": medical_info.medications if medical_info else '',
            "surgeries": medical_info.surgeries if medical_info else '',
            "immunizations": medical_info.immunizations if medical_info else '',
            "family_history": medical_info.family_history if medical_info else '',
            "lifestyle": medical_info.lifestyle if medical_info else '',
            "emergency_contact_name": medical_info.emergency_contact_name if medical_info else '',
            "emergency_contact_phone": medical_info.emergency_contact_phone if medical_info else '',
            "emergency_contact_relation": medical_info.emergency_contact_relation if medical_info else '',
            "last_visit": medical_info.last_visit if medical_info else '',
            "notes": medical_info.notes if medical_info else '',
            "info_other_doctor": medical_info.info_other_doctor if medical_info else '',
